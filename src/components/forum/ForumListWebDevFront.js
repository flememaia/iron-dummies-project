...
class ForumListWebDevFront extends Component {
  state = {
    _id: "",
    username: "",
    points: "",
  ...
  componentDidMount = async () => {
    try {
      ...
      const newResponseWebDevFront = []
      newResponse.map(object => {
        if(object.skilltype === "WebDev Front"){
          newResponseWebDevFront.push(object)
        }
        return newResponseWebDevFront   
      })
      console.log(newResponseWebDevFront)
      this.setState({ allUsers: [...newResponseWebDevFront] });
    } catch (err) {
      console.error(err);
    }
  };
...
export default ForumListWebDevFront;
